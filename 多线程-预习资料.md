...
我们来谈一下synchronized它的一些特性。如果说你每次都定义一个锁的对象Object o 把它new出来那加锁的时候太麻烦每次都要new一个新的对象出来，所以呢，有一个简单的方式就是**synchronized(this)**锁定当前对象就行
...
我们知道静态方法static是没有this对象的，你不需要new出一个对象来就能执行这个方法，但如果这个这个上面加一个synchronized的话就代表synchronized(T.class)。这里这个==synchronized(T.class)锁的就是T类的对象==
...
另外这个之外还可以加synchronized，加了synchronized就没有必要在加volatile了，因为==synchronized既保证了原子性，又保证了可见性。==
