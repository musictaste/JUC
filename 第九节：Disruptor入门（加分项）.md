...
数组的扩容，每添加一个元素，数组要copy扩容

Disruptor是数组实现的

**效率高的原因：因为是数据，写入快；只维护一个sequence指针；直接覆盖旧数据；多线程同步只维护sequence**
